...
vector<__int64> randomCircuitOneshot(unsigned int n, unsigned int depth, unsigned int core) {
	std::mt19937 mt(0);
	...
	string fname = "cputime" + to_string(core) + ".txt";

	std::fstream ofs(fname, std::ios::app);
	ofs << n << " ";
	ofs.close();
	...
	for (int d = 0; d < depth; d++) {
		...
		std::fstream ofsa(fname, std::ios::app);
		ofsa << time << " ";
		ofsa.close();
	}
	mc->close();
	std::fstream ofse(fname, std::ios::app);
	ofse << std::endl;
	ofse.close();
...
int main(int argc, char** argv) {
	...
	auto time = randomCircuitOneshot(n, depth,nth);
	return 0;
}